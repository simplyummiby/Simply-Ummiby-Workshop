(() => {
  ...
  function renderProductRecipeSummary(recipeId, materials = []) {
    ...
    const recipeRows = (recipe.materials || []).map(row => `<li><strong>${escapeHTML(inventoryItemById(row.inventoryItemId)?.name || row.name)}</strong><span>${escapeHTML(recipeComponentDisplay(row))}</span></li>`).join("");
  ...
  function normalizeRecipeComponents(recipe) {
    const existing = Array.isArray(recipe?.materials) ? recipe.materials : [];
    if (existing.some(row => row.inventoryItemId || row.usageType)) return existing.map(row => ({
      id: row.id || uid("recipe-component"),
      inventoryItemId: row.inventoryItemId || "",
      name: row.name || inventoryItemById(row.inventoryItemId)?.name || "",
      usageType: row.usageType || "count",
      quantity: Number(row.quantityValue ?? row.quantity ?? 1) || 1,
      pieces: Number(row.pieces || 1) || 1,
    ...
    return existing.map((row,index) => {
      ...
      const isCut=definition.kind==="length";
      const quantityText=String(row.quantity||"");
      const piecesMatch=quantityText.match(/(\d+)\s*[×x]\s*([\d.]+)/i);
      const pieces=isCut ? Number(piecesMatch?.[1]||1) : 1;
  ...
  function syncRecipeComponentRow(row) {
    const type=row.querySelector('[data-field="usageType"]')?.value||"count";
    row.querySelectorAll('.component-count-field').forEach(el=>el.hidden=type!=="count");
    row.querySelectorAll('.component-cut-field').forEach(el=>el.hidden=type!=="cut");
    const selectedId=row.querySelector('[data-field="inventoryItemId"]')?.value||"";
    const dynamicYarn=selectedId==="__order_color_yarn__";
    const item=inventoryItemById(selectedId);
    const linked=dynamicYarn||Boolean(item);
    const state=row.querySelector('.recipe-link-state');
    if(state){state.textContent=linked?(dynamicYarn?"Linked by order color":"Linked"):"Inventory link needed";state.classList.toggle("linked",linked);state.classList.toggle("unlinked",!linked);}
    const availability=row.querySelector('[data-role="availability"]');
    if(availability) availability.textContent=dynamicYarn ? "Matched to the order color during production" : item ? (isYarnOrCord(item)&&item.yarnTrackingMode==="precise"?`${formatInventoryNumber(item.yarnRemainingLength)} ${item.yarnLengthUnit||"yd"} available`:item.tracking==="quantity"?`${formatInventoryNumber(item.quantity)} available`:item.condition||"Condition tracked") : "Not linked to inventory";
    const summary=row.querySelector('[data-role="summary"]');
    if(summary){
      if(type==="cut") { const pieces=Number(row.querySelector('[data-field="pieces"]')?.value||0); const each=Number(row.querySelector('[data-field="lengthEach"]')?.value||0); const unit=row.querySelector('[data-field="lengthUnit"]')?.value||"cm"; summary.textContent=pieces&&each?`${pieces} × ${formatInventoryNumber(each)} ${unit} = ${formatInventoryNumber(pieces*each)} ${unit}`:"Enter the pieces and length for this cut."; }
      else { const qty=Number(row.querySelector('[data-field="quantity"]')?.value||0); summary.textContent=qty?`${formatInventoryNumber(qty)} ${item?.name||"item"}${qty===1?"":"s"} per product`:"Enter a quantity."; }
    }
  }
  ...
  function recipeComponentDisplay(component) {
    if (component?.usageType === "cut") return `${component.pieces} × ${formatInventoryNumber(component.lengthEach)} ${component.lengthUnit || "cm"}`;
    if (component?.usageType === "count") return `${formatInventoryNumber(component.quantityValue ?? component.quantity)}${component.description ? ` · ${component.description}` : ""}`;
  ...
  function showRecipeEditor(recipeId="") {
    ...
    showModal(recipe?"Edit Recipe":"Add Recipe",`<form id="recipeEditorForm" class="recipe-editor-form" novalidate>
      ...
      <section class="product-form-section recipe-worktable-section"><div class="product-section-heading"><span>Worktable</span><h4>Build & Pack Components</h4><p>Keep production materials separate from the supplies used at the packing table.</p></div>
        <div class="recipe-phase-section"><div class="recipe-phase-heading"><div><span>Phase 1</span><h5>Manufacturing Components</h5><p>Cord, rings, dowels, end caps, and anything consumed while making the product.</p></div></div><div id="manufacturingComponentsList" class="recipe-components-list compact">${manufacturingComponents.map(component=>recipeComponentRow(component,"manufacturing")).join("")}</div><div class="recipe-phase-footer"><button type="button" id="addManufacturingComponent" class="button secondary small">+ Add Manufacturing Item</button></div></div>
        <div class="recipe-phase-section packaging"><div class="recipe-phase-heading"><div><span>Phase 2</span><h5>Packaging Components</h5><p>Mailers, wrapping string, labels, stickers, care sheets, and other packing supplies.</p></div></div><div id="packagingComponentsList" class="recipe-components-list compact">${packagingComponents.map(component=>recipeComponentRow(component,"packaging")).join("")}</div><div class="recipe-phase-footer"><button type="button" id="addPackagingComponent" class="button secondary small">+ Add Packaging Item</button></div></div>
  ...
  function saveRecipeEditor(recipeId="") {
    ...
    try {
      ...
      data.activity.unshift({text:`${existing?"Updated":"Added"} recipe: ${updated.title}`,time:"Just now"});
      saveData();
      hideModal();
